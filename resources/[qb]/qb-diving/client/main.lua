...
RegisterNetEvent("qb-diving:client:setoxygenlevel", function()
    if oxgenlevell == 0 then
       ...
       QBCore.Functions.Notify('Bình Lặn đã được bơm đầy khí', 'success')
    else
        QBCore.Functions.Notify('bình lặn chỉ có thể bơm khi đã hết, vẫn còn '..oxgenlevell..'%', 'error')
    end
end)
    ...
    if iswearingsuit == false then
        if oxgenlevell > 0 then
            ...
            if not IsPedSwimming(ped) and not IsPedInAnyVehicle(ped) then
                ...
                QBCore.Functions.Notify('Bạn cần Bình Lặn', 'error')
            end

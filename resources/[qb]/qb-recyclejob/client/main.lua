  ...
  if Config.UseTarget then
    ...
    entranceZone:onPlayerInOut(function(isPointInside)
      if isPointInside then
        exports['qb-core']:DrawText('[E] Vào trong nhà kho', 'left')
      else
...
local function RegisterExitTarget()
  ...
  if Config.UseTarget then
    ...
    exitZone:onPlayerInOut(function(isPointInside)
      if isPointInside then
        exports['qb-core']:DrawText('[E] Ra khỏi nhà kho', 'left')
      else
        exports['qb-core']:HideText()
      end
...
local function GetDutyTargetText()
  local text = onDuty and '[E] Dừng Việc' or '[E] Nhận Việc'
  return text
end
  ...
  if Config.UseTarget then
    ...
    deliveryZone:onPlayerInOut(function(isPointInside)
      if isPointInside and carryPackage then
        exports['qb-core']:DrawText('[E] Đóng gói', 'left')
      else
        exports['qb-core']:HideText()
      end
...
local function SetLocationBlip()
  ...
  AddTextComponentString('Trung Tấm Phế Liệu')
...
function RegisterPickupTarget(coords)
  ...
  if Config.UseTarget then
    ...
    pickupZone:onPlayerInOut(function(isPointInside)
      if isPointInside then
        exports['qb-core']:DrawText('[E] Nhận Thùng Hàng', 'left')
      else
        exports['qb-core']:HideText()
      end
...
RegisterNetEvent('qb-recyclejob:client:target:toggleDuty', function()
  onDuty = not onDuty
  if onDuty then
    QBCore.Functions.Notify('Bạn đã nhận công việc', 'success')
    GetRandomPackage()
  else
    QBCore.Functions.Notify('bạn đã dừng công việc', 'error')
    DestroyPickupTarget()
  end
...
RegisterNetEvent('qb-recyclejob:client:target:pickupPackage', function()
  ...
  QBCore.Functions.Progressbar('pickup_reycle_package', 'Đang Lấy Thùng Hàng', Config.PickupActionDuration, false, true, {
...
RegisterNetEvent('qb-recyclejob:client:target:dropPackage', function()
  ...
  ScrapAnim()
  DestroyDeliveryTarget()
  QBCore.Functions.Progressbar('deliver_reycle_package', 'Giao Hàng', Config.DeliveryActionDuration, false, true, {
      disableMovement = true,
      disableCarMovement = true,
      disableMouse = false,
      disableCombat = true

        ...
        if LocalPlayer.state.isLoggedIn and not keyPressed and GetLastInputMethod(0) then
            ...
            if next(keyMeta) ~= nil then
                if keyMeta[v[2]]["command"] ~= "" then
                    ...
                    QBCore.Functions.Notify('phím này chưa có lệnh nào, /binds để cài phím tắt', 'primary', 4000)
                end
            else
                QBCore.Functions.Notify('Bạn chưa cài lệnh nào, /binds để cài phím tắt', 'primary', 4000)
            end
        end
    end, false)
...
RegisterNUICallback('save', function(data, cb)
    ...
    QBCore.Functions.Notify('Phím Tắt Đã Được Lưu', 'success')

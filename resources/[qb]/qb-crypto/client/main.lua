			...
			if dist < 15 then
				...
				if dist < 1.5 then
					if not Crypto.Exchange.RebootInfo.state then
						DrawText3Ds(Crypto.Exchange.coords, '~g~E~w~ - sử dụng USB')
						...
						if IsControlJustPressed(0, 38) then
							QBCore.Functions.TriggerCallback('qb-crypto:server:HasSticky', function(HasItem)
								if HasItem then
									...
									QBCore.Functions.Notify('bạn không còn thanh điện tử', 'error')
								end
							end)
						end
					else
						DrawText3Ds(Crypto.Exchange.coords, 'Đang khởi động hệ thống - '..Crypto.Exchange.RebootInfo.percentage..'%')
					end
				else

...
QBCore.Commands.Add("checkcryptoworth", "", {}, false, function(source)
    local src = source
    TriggerClientEvent('QBCore:Notify', src, "tiền điện tử đang có giá là: $"..Crypto.Worth["qbit"])
end)
...
QBCore.Commands.Add("crypto", "", {}, false, function(source)
    ...
    TriggerClientEvent('QBCore:Notify', src, "Bạn Đang Có: "..Player.PlayerData.money.crypto.." QBit, với tổng giá trị là: $"..MyPocket..",-")
end)
...
RegisterServerEvent('qb-crypto:server:ExchangeFail', function()
    ...
    if ItemData ~= nil then
        ...
        TriggerClientEvent('QBCore:Notify', src, "Thanh Điện Tử Bị Lỗi", 'error')
    end
end)
...
RegisterServerEvent('qb-crypto:server:ExchangeSuccess', function(LuckChance)
    ...
    if ItemData ~= nil then
        ...
        TriggerClientEvent('QBCore:Notify', src, "Bạn Đã Đào Thành Công: "..Amount.." QBit(\'s)", "success", 3500)
        TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items["cryptostick"], "remove")
        TriggerClientEvent('qb-phone:client:AddTransaction', src, Player, {}, "Bạn Có "..Amount.." Qbit('s) Trong Tài Khoản!", "Credit")
    end
end)
...
QBCore.Functions.CreateCallback('qb-crypto:server:BuyCrypto', function(source, cb, data)
    ...
    if Player.PlayerData.money.bank >= total_price then
        ...
        TriggerClientEvent('qb-phone:client:AddTransaction', source, Player, data, "Bạn Đã Mua "..tonumber(data.Coins).." Qbit('s)!", "Credit")
...
QBCore.Functions.CreateCallback('qb-crypto:server:SellCrypto', function(source, cb, data)
    ...
    if Player.PlayerData.money.crypto >= tonumber(data.Coins) then
        ...
        TriggerClientEvent('qb-phone:client:AddTransaction', source, Player, data, "Bạn Đã Bán "..tonumber(data.Coins).." Qbit('s)!", "Depreciation")
...
QBCore.Functions.CreateCallback('qb-crypto:server:TransferCrypto', function(source, cb, data)
    ...
    if Player.PlayerData.money.crypto >= tonumber(data.Coins) then
        ...
        if result[1] ~= nil then
            ...
            Player.Functions.RemoveMoney('crypto', tonumber(data.Coins))
            TriggerClientEvent('qb-phone:client:AddTransaction', source, Player, data, "Bạn Đã Chuyển "..tonumber(data.Coins).." Qbit('s)!", "Depreciation")
            ...
            if Target ~= nil then
                ...
                TriggerClientEvent('qb-phone:client:AddTransaction', Target.PlayerData.source, Player, data, "Bạn Có "..tonumber(data.Coins).." Qbit('s)!", "Credit")
            else

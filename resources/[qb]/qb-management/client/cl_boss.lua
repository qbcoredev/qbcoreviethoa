...
RegisterNetEvent('qb-bossmenu:client:OpenMenu', function()
    ...
    local bossMenu = {
        {
            header = "Quản lý công ty - " .. string.upper(PlayerJob.label),
            icon = "fa-solid fa-circle-info",
            isMenuHeader = true,
        },
        {
            header = "Danh Sách Nhân Viên",
            txt = "Danh Sách Nhân Viên",
        ...
        {
            header = "Thuê Nhân Viên",
            txt = "Thuê Người Đứng Gần Trở Thành Nhân Viên",
...
RegisterNetEvent('qb-bossmenu:client:employeelist', function()
    local EmployeesMenu = {
        {
            header = "Quản Lý Nhân Viên - " .. string.upper(PlayerJob.label),
            isMenuHeader = true,
            icon = "fa-solid fa-circle-info",
        },
    }
...
RegisterNetEvent('qb-bossmenu:client:HireMenu', function()
    local HireMenu = {
        {
            header = "Thuê Nhân Viên - " .. string.upper(PlayerJob.label),
            isMenuHeader = true,
            icon = "fa-solid fa-circle-info",
        },
    }
    QBCore.Functions.TriggerCallback('qb-bossmenu:getplayers', function(players)
        for _, v in pairs(players) do
            if v and v ~= PlayerId() then
                HireMenu[#HireMenu + 1] = {
                    header = v.name,
                    txt = "Số Căn Cước: " .. v.citizenid .. " - ID: " .. v.sourceplayer,
...
RegisterNetEvent('qb-bossmenu:client:SocetyDeposit', function(money)
    local deposit = exports['qb-input']:ShowInput({
        header = "Cất Tiền <br> Số Dư CTY: $" .. money,
        submitText = "Xác Nhận",
        inputs = {
            {
                type = 'number',
...
RegisterNetEvent('qb-bossmenu:client:SocetyWithDraw', function(money)
    local withdraw = exports['qb-input']:ShowInput({
        header = "Rút Tiền <br> Số Dư CTY: $" .. money,
        submitText = "Xác Nhận",
        inputs = {
            {
                type = 'number',
                            ...
                            if #(pos - coords) < 5.0 then
                                ...
                                if #(pos - coords) <= 1.5 then
                                    ...
                                    if not shownBossMenu then
                                        exports['qb-core']:DrawText('[E] Quản Lý Công Ty', 'left')
                                        shownBossMenu = true
                                    end

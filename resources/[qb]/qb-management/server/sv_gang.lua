...
RegisterNetEvent("qb-gangmenu:server:withdrawMoney", function(amount)
	...
	if RemoveGangMoney(gang, amount) then
		...
		TriggerEvent('qb-log:server:CreateLog', 'gangmenu', 'Withdraw Money', 'yellow', Player.PlayerData.charinfo.firstname .. ' ' .. Player.PlayerData.charinfo.lastname .. ' đã rút thành công $' .. amount .. ' (' .. gang .. ')', false)
		TriggerClientEvent('QBCore:Notify', src, "đã rút thành công: $" ..amount, "success")
	else
		TriggerClientEvent('QBCore:Notify', src, "Không đủ tiền trong quỹ!", "error")
	end
...
RegisterNetEvent("qb-gangmenu:server:depositMoney", function(amount)
	...
	if Player.Functions.RemoveMoney("cash", amount) then
		...
		TriggerEvent('qb-log:server:CreateLog', 'gangmenu', 'Deposit Money', 'yellow', Player.PlayerData.charinfo.firstname .. ' ' .. Player.PlayerData.charinfo.lastname .. ' đã cất thành công $' .. amount .. ' (' .. gang .. ')', false)
		TriggerClientEvent('QBCore:Notify', src, "đã cất thành công: $" ..amount, "success")
	else
		TriggerClientEvent('QBCore:Notify', src, "Bạn không đủ tiền để cất!", "error")
	end
...
RegisterNetEvent('qb-gangmenu:server:GradeUpdate', function(data)
	...
	if data.grade > Player.PlayerData.gang.grade.level then TriggerClientEvent('QBCore:Notify', src, "Bạn không thể thăng hạng lên thứ hạng này!", "error") return end

	if Employee then
		if Employee.Functions.SetGang(Player.PlayerData.gang.name, data.grade) then
			TriggerClientEvent('QBCore:Notify', src, "Đã thăng cấp thành công!", "success")
			TriggerClientEvent('QBCore:Notify', Employee.PlayerData.source, "Bạn đã được thăng cấp lên " ..data.gradename..".", "success")
		else
			TriggerClientEvent('QBCore:Notify', src, "cấp độ không tồn tại.", "error")
		end
	else
		TriggerClientEvent('QBCore:Notify', src, "Công dân không online.", "error")
	end
	TriggerClientEvent('qb-gangmenu:client:OpenMenu', src)
end)
...
RegisterNetEvent('qb-gangmenu:server:FireMember', function(target)
	...
	if Employee then
		if target ~= Player.PlayerData.citizenid then
			if Employee.PlayerData.gang.grade.level > Player.PlayerData.gang.grade.level then TriggerClientEvent('QBCore:Notify', src, "Không thể đuổi người này!", "error") return end
			if Employee.Functions.SetGang("none", '0') then
				...
				TriggerClientEvent('QBCore:Notify', src, "Đã đuổi người này!", "success")
				TriggerClientEvent('QBCore:Notify', Employee.PlayerData.source , "Bạn đã bị khai trừ khỏi gang!", "error")
			else
				TriggerClientEvent('QBCore:Notify', src, "lỗi.", "error")
			end
		else
			TriggerClientEvent('QBCore:Notify', src, "không thể đuổi bản thân!", "error")
		end
	else
		...
		if player[1] ~= nil then
			...
			if Employee.gang.grade.level > Player.PlayerData.job.grade.level then TriggerClientEvent('QBCore:Notify', src, "Không thể đuổi người này!", "error") return end
			local gang = {}
			gang.name = "none"
			...
			TriggerClientEvent('QBCore:Notify', src, "Người này không trực tuyến.", "error")
		end
	end
	TriggerClientEvent('qb-gangmenu:client:OpenMenu', src)
end)
...
RegisterNetEvent('qb-gangmenu:server:HireMember', function(recruit)
	...
	if Target and Target.Functions.SetGang(Player.PlayerData.gang.name, 0) then
		TriggerClientEvent('QBCore:Notify', src, "Bạn đã thuê " .. (Target.PlayerData.charinfo.firstname .. ' ' .. Target.PlayerData.charinfo.lastname) .. " trở thành " .. Player.PlayerData.gang.label .. "", "success")
		TriggerClientEvent('QBCore:Notify', Target.PlayerData.source , "bạn đã được tuyển và trở thành " .. Player.PlayerData.gang.label .. "", "success")

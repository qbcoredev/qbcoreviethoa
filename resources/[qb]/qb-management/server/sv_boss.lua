...
RegisterNetEvent("qb-bossmenu:server:withdrawMoney", function(amount)
	...
	if RemoveMoney(job, amount) then
		...
		TriggerClientEvent('QBCore:Notify', src, "Bạn đã rút: $" ..amount, "success")
	else
		TriggerClientEvent('QBCore:Notify', src, "Bạn không có đủ tiền trong tài khoản!", "error")
	end
...
RegisterNetEvent("qb-bossmenu:server:depositMoney", function(amount)
	...
	if Player.Functions.RemoveMoney("cash", amount) then
		...
		TriggerClientEvent('QBCore:Notify', src, "Bạn đã cất: $" ..amount, "success")
	else
...
RegisterNetEvent('qb-bossmenu:server:GradeUpdate', function(data)
	...
	if data.grade > Player.PlayerData.job.grade.level then TriggerClientEvent('QBCore:Notify', src, "Bạn không thể thăng hạng lên thứ hạng này!", "error") return end
	
	if Employee then
		if Employee.Functions.SetJob(Player.PlayerData.job.name, data.grade) then
			TriggerClientEvent('QBCore:Notify', src, "Thăng Chức Thành Công!", "success")
			TriggerClientEvent('QBCore:Notify', Employee.PlayerData.source, "Bạn đã được thăng chứ làm" ..data.gradename..".", "success")
		else
			TriggerClientEvent('QBCore:Notify', src, "Đã Có Lỗi Xảy Ra.", "error")
		end
	else
...
RegisterNetEvent('qb-bossmenu:server:FireEmployee', function(target)
	...
	if Employee then
		if target ~= Player.PlayerData.citizenid then
			if Employee.PlayerData.job.grade.level > Player.PlayerData.job.grade.level then TriggerClientEvent('QBCore:Notify', src, "Bạn không thể sa thải người này!", "error") return end
			if Employee.Functions.SetJob("unemployed", '0') then
				TriggerEvent("qb-log:server:CreateLog", "bossmenu", "Job Fire", "red", Player.PlayerData.charinfo.firstname .. " " .. Player.PlayerData.charinfo.lastname .. ' successfully fired ' .. Employee.PlayerData.charinfo.firstname .. " " .. Employee.PlayerData.charinfo.lastname .. " (" .. Player.PlayerData.job.name .. ")", false)
				TriggerClientEvent('QBCore:Notify', src, "Đã Đuổi Việc!", "success")
				TriggerClientEvent('QBCore:Notify', Employee.PlayerData.source , "Bạn Đã Bị Đuổi Việc", "error")
			else
				TriggerClientEvent('QBCore:Notify', src, "Error..", "error")
			end
		else
			TriggerClientEvent('QBCore:Notify', src, "Bạn không thể tự sa thải mình", "error")
		end
	else
		...
		if player[1] ~= nil then
			...
			if Employee.job.grade.level > Player.PlayerData.job.grade.level then TriggerClientEvent('QBCore:Notify', src, "Bạn không thể sa thải người này!", "error") return end
			local job = {}
			job.name = "unemployed"
			...
			TriggerClientEvent('QBCore:Notify', src, "Nhân viên bị sa thải!", "success")
			TriggerEvent("qb-log:server:CreateLog", "bossmenu", "Job Fire", "red", Player.PlayerData.charinfo.firstname .. " " .. Player.PlayerData.charinfo.lastname .. ' successfully fired ' .. Employee.PlayerData.charinfo.firstname .. " " .. Employee.PlayerData.charinfo.lastname .. " (" .. Player.PlayerData.job.name .. ")", false)
		else
			TriggerClientEvent('QBCore:Notify', src, "Người này không trực tuyến.", "error")
		end
...
RegisterNetEvent('qb-bossmenu:server:HireEmployee', function(recruit)
	...
	if Target and Target.Functions.SetJob(Player.PlayerData.job.name, 0) then
		TriggerClientEvent('QBCore:Notify', src, "Bạn đã thuê " .. (Target.PlayerData.charinfo.firstname .. ' ' .. Target.PlayerData.charinfo.lastname) .. " come " .. Player.PlayerData.job.label .. "", "success")
		TriggerClientEvent('QBCore:Notify', Target.PlayerData.source , "Bạn đã được thuê với tư cách là " .. Player.PlayerData.job.label .. "", "success")

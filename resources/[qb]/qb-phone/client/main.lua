...
RegisterCommand('phone', function()
    ...
    if not PhoneData.isOpen and LocalPlayer.state.isLoggedIn then
        if not PlayerData.metadata['ishandcuffed'] and not PlayerData.metadata['inlaststand'] and not PlayerData.metadata['isdead'] and not IsPauseMenuActive() then
            ...
            QBCore.Functions.Notify("Hành động không khả dụng vào lúc này..", "error")
        end
    end
end)
...
RegisterNUICallback('track-vehicle', function(data, cb)
    ...
    if findVehFromPlateAndLocate(veh.plate) then
        QBCore.Functions.Notify("Phương tiện của bạn đã được đánh dấu", "success")
    else
        QBCore.Functions.Notify("Không thể định vị phương tiện này", "error")
    end
    cb("ok")
end)
...
RegisterNUICallback('FetchVehicleScan', function(_, cb)
    ...
    QBCore.Functions.TriggerCallback('qb-phone:server:ScanPlate', function(result)
	    ...
	    if QBCore.Shared.Vehicles[vehname] ~= nil then
                ...
                result.label = 'Thương hiệu không xác định..'
            end
            cb(result)
        end, plate)
    end, plate)
...
RegisterNUICallback('RaceDistanceCheck', function(data, cb)
    QBCore.Functions.TriggerCallback('qb-lapraces:server:GetRacingData', function(RaceData)
        ...
        if dist <= 115.0 then
            ...
            QBCore.Functions.Notify('Bạn đang ở quá xa cuộc đua. GPS đã được thiết lập cho cuộc đua.', 'error', 5000)
...
RegisterNUICallback('SetHouseLocation', function(data, cb)
    ...
    QBCore.Functions.Notify("GPS đã được đặt thành " .. data.HouseData.HouseData.adress .. "!", "success")
    cb("ok")
end)
...
RegisterNUICallback('SetGPSLocation', function(data, cb)
    ...
    QBCore.Functions.Notify('GPS đã được thiết lập!', 'success')
    cb("ok")
end)
...
RegisterNUICallback('CanTransferMoney', function(data, cb)
    ...
    if (PlayerData.money.bank - amount) >= 0 then
		...
		SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { timeout=3000, title = "Bank", text = "Tài khoản không tồn tại!", icon = "fas fa-university", color = "#ff0000", }, })
...
RegisterCommand('ping', function(_, args)
    if not args[1] then
        QBCore.Functions.Notify("Bạn cần nhập ID người chơi", "error")
    else
...
RegisterNetEvent('qb-phone:client:TransferMoney', function(amount, newmoney)
    ...
    SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { title = "QBank", text = "&#36;"..amount.."đã được thêm vào tài khoản của bạn!", icon = "fas fa-university", color = "#8c7ae6", }, })
        ...
        if src ~= MyPlayerId then
            SendNUIMessage({
                ...
                PhoneNotify = {
                    ...
                    text = "Một tweet mới đã được đăng.",
                    icon = "fab fa-twitter",
                    color = "#1DA1F2",
                },
            })
...
RegisterNetEvent('qb-phone:client:NewMailNotify', function(MailData)
    SendNUIMessage({
        ...
        PhoneNotify = {
            title = "Mail",
            text = "Bạn đã nhận được một thư mới từ "..MailData.sender,
...
RegisterNetEvent('qb-phone:client:UpdateAdverts', function(Adverts, LastAd)
    ...
    SendNUIMessage({
        ...
        PhoneNotify = {
            title = "Advertisement",
            text = "Một quảng cáo mới đã được đăng bởi "..LastAd,
            icon = "fas fa-ad",
            color = "#ff8f1a",
            timeout = 2500,
        },
...
RegisterNetEvent('qb-phone:client:BillingEmail', function(data, paid, name)
    if paid then
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            ...
            message = 'Hóa đơn đã được thanh toán từ '..name..' với số tiền $'..data.amount,
        })
    else
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            sender = 'Billing Department',
            subject = 'Hóa đơn bị Từ chối',
            message = 'Hóa đơn đã bị Từ chối bởi '..name..' với số tiền $'..data.amount,
        })
    end
end)
        ...
        if PhoneData.isOpen then
            if SenderNumber ~= PhoneData.PlayerData.charinfo.phone then
                ...
                SendNUIMessage({
                    ...
                    PhoneNotify = {
                        title = "Whatsapp",
                        text = "Đã gửi tin nhắn cho bản thân",
                        icon = "fab fa-whatsapp",
                        color = "#25D366",
                        timeout = 4000,
                    },
...
RegisterNetEvent('qb-phone:client:RemoveBankMoney', function(amount)
    if amount > 0 then
        SendNUIMessage({
            ...
            PhoneNotify = {
                title = "Bank",
                text = "$"..amount.." đã bị trừ khỏi ngân hàng!",
                icon = "fas fa-university",
                color = "#ff002f",
                timeout = 3500,
            },
...
RegisterNetEvent('qb-phone:client:AddNewSuggestion', function(SuggestionData)
    ...
    SendNUIMessage({
        ...
        PhoneNotify = {
            title = "Phone",
            text = "Bạn có một đề xuất liên hệ mới!",
            icon = "fa fa-phone-alt",
            color = "#04b543",
            timeout = 1500,
        },
